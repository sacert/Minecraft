...
bool Chunk::checkNeighbour(int x, int y, int z) {
    ...
    if (x > 15 && !neighbours[0].isEmpty()) 
        return neighbours[0].getBlockWorld(Coordinates(0, y, z)) == BlockType::AIR;

    // left
    if (x < 0 && !neighbours[1].isEmpty()) 
        return neighbours[1].getBlockWorld(Coordinates(15, y, z)) == BlockType::AIR;

    // front
    if (z > 15 && !neighbours[2].isEmpty()) 
        return neighbours[2].getBlockWorld(Coordinates(x, y, 0)) == BlockType::AIR;

    // back
    if (z < 0 && !neighbours[3].isEmpty()) 
        return neighbours[3].getBlockWorld(Coordinates(x, y, 15)) == BlockType::AIR;
...
BlockType Chunk::getBlockWorld(Coordinates blockCoord) {
